<template>
  <div
    class='bus-font'
    :style="windowSize.x < 600 ? 'margin-bottom: 55px;': 'margin-bottom: 64px;'"
  >
    <v-navigation-drawer
      clipped
      fixed
      ...
      <v-container>
        <v-list>
          ...
          >
            ...
            <v-list-tile v-for='subItem in item.items' :key='subItem.title' @click="subItemAction(subItem, item.action)">
              <v-list-tile-content>
                <v-list-tile-title>
    ...
    <v-toolbar
      ...
      class='elevation-5 px-0' style="z-index: 6; overflow: hidden"
    >
      <v-toolbar-side-icon class="displayNoneTiny white--text" @click.stop='drawer = !drawer' :dark='!logo' id="toolbarDrawer"></v-toolbar-side-icon>
      <v-layout row class="px-2 mx-0">
        ...
        <v-flex xs2 sm1 v-if="
          windowSize.x > 600 &&
          sidePanelItems.find((item) => item.title === 'Cybermonday')
        ">
          <v-toolbar-items>
            <v-btn
              flat :dark='!logo'
              small
              class="white--text"
              @click="$router.push({ name: 'gifcard' })"
            >
            ...
            <v-flex md3 xs12 class="logo-vb">
              <v-toolbar-items>
                <v-btn flat :dark='!logo' small @click="$router.push({path: '/'})">
                  <img v-if='logo' src='/static/imgs/logos/Logo2.png' alt class='principalLogo'>
                </v-btn>
              </v-toolbar-items>
            </v-flex>
          </v-layout>
          ...
          <v-layout row style="height: 100%;">
            <template v-if="windowSize.x <= 600">
              <v-flex xs6 offset-xs6>
                <v-toolbar-items>
                  <v-btn flat :dark='!logo' @click.stop='drawer = !drawer' class='white--text' style="width: 50px; min-width: 0px">
