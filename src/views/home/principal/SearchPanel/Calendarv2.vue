  ...
  <v-layout row wrap class="font">
    <v-flex xs12 style="position: relative">
      <v-menu lazy :close-on-content-click="true" transition="scale-transition" offset-y full-width
        ...
        <v-date-picker @input="console" min="2017-01-24" v-model="newdata" :allowed-dates="enableToDate" color="primary"
...
export default {
  ...
  computed: {
    ...
    userCalendar: {
      get() {
        if (!this.value) {
          this.formattedDate = this.newdata
          return null
        }
        const format = 'dddd LL'
        this.formattedDate = moment(this.newdata).format(format)
        return this.value
      },
      set(value) {
