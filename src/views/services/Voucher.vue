<template>
  <div>
    <v-container class="center">
              ...
              <h3 class="mb-2">Datos de la compra:</h3>
                ...
                class="elevation-1">
                  <template slot="items" slot-scope="props">
                    ...
                    <td class="text-xs-right">{{props.item.montoFormateado}}</td>
                    <td class="text-xs-center">{{props.item.codigoTransbank}}</td>
                    <td class="text-xs-right">{{fechaFormateada}}</td>
                    <td class="text-xs-right">{{props.item.tipoPagoFormateado}}</td>
      ...
      <v-btn style="background-color:#a0a0a0;" class="white--text mt-5" @click="$router.push({path: '/'})" v-lang.back></v-btn>
    </v-container>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    toPDF (response) {
      ...
      const blob = new Blob([this.toUint8Array(response.archivo)], { type: `application/${tipo}` })
