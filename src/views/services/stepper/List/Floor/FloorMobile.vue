...
<div>
  ...
  <v-card flat class="ml-4 mr-4 mb-4">
          ...
          <v-container v-else-if="bus.grilla.length > 0" :class="{'pa-0': isXs}">
            <v-layout row wrap :justify-center="floorArray.length === 1">
              <v-flex xs12>
                <div>
                  <v-layout row wrap :justify-center="floorArray.length === 1">
                    <v-flex xs6 sm6 class="d-flex" v-for="(selectedFloor, index) in floorArray" :key="index">
                      <v-btn
                        class="floors"
                        :class="{'floor-selected': clickedFloor === selectedFloor}"
                        flat
                        color="primary"
                        @click="clickedFloor = selectedFloor"
                      >
                        <h3>{{translate('floor')}} {{selectedFloor + 1}}</h3>
                      </v-btn>
                    </v-flex>
                  </v-layout>

                  <div  v-for="(selectedFloor, index) in floorArray" :key="index">
                    <div v-show="clickedFloor === selectedFloor" :style="{width: clickedFloor === selectedFloor? '100%' : 0}">
            ...
            <v-layout row text-xs-left justify-center>
              <v-flex>
                <div v-for="(item, index) in seatsImg" :key="index">
                  <h3 class="d-block service-title">
                  <img
                    height="22"
                    width="22"
                    :src="
