  <template>
    <div>
      <v-card flat hover class="mt-2 transparentCard">
        <v-container fluid>
          <h3 class="filter-title" v-lang.company></h3>
          <v-radio-group v-model="company" :mandatory="false" @change="setFilter">
            <v-radio color="primarylight" v-for="(item, index) in companies" :key="index" :label="item.name" :value="item.id"></v-radio>
          </v-radio-group>
        </v-container>
      </v-card>
    </div>
...
export default {
  ...
  methods: {
    setFilter () {
      this.$store.dispatch('SET_USER_FILTER', {filter: this.company, type: 'selectedCompany'})
    }
  },
  computed: {
    ...mapGetters({
