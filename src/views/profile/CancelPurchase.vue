<template>
  <div>
    <p class="subheading">
      Debe seleccionar el o los boletos que desea anular para la devolución de su dinero.
      La devolución debe ser realizada por lo menos 4 hrs. antes de la salida del servicio.
    </p>
    <form>
    ...
    >
      ...
      <template slot="items" slot-scope="props">
        ...
        <td class="text-xs-left pa-1">{{ props.item.nombreTerminalDestino }}</td>
        <td class="text-xs-center">{{ props.item.asiento }}</td>
        <td class="text-xs-left pa-2">{{ props.item.total | currency }}</td>
        <td class="text-xs-left">
          <v-btn flat icon color="primary" @click="getTicketInfo(props.item.boleto)">
    ...
    <i class="subheading">
      *Si la compra se realizo con tarjeta de credito, se generara una reversa en su cuenta, si se realizo con tarjeta de debito, se realizara
      una transferencia a la cuenta que entregue para este fin. La devolución o reversa se hara efectiva en un plazo maximo de 6 dias
    </i>
